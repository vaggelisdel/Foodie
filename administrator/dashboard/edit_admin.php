<?php
	...
	if ($_SESSION['admin_authorised'] == 0){
		...
		if (isset($_GET["adminid"])){
			...
			$result = $connect->query("SELECT * FROM admins WHERE `AdminID`='$adminid'");
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"><a href="all_admins.php">Admins</a></li>
            ...
            <div class="card card-danger">
              <div class="card-header">
				...
				<div class="card-tools">
                  <form id="deleteadmin_form" action="actions/edit_admin_actions.php" method="POST">
					  <input type="hidden" name="adminid" value="<?= $admin["AdminID"]; ?>"/>
					  <input type="hidden" name="action" id="action" value=""/>
					  <button type="button" name="delete_admin_btn" onclick="deleteadmin_function()" class="btn btn-default float-right"><i class="fa fa-trash"></i> Delete the admin</button>
				  </form>
				</div>
                </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form" action="actions/edit_admin_actions.php" method="POST">
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputName">Name</label>
                    <input required type="text" class="form-control" name="user_name" value="<?= $admin["Name"]; ?>" id="exampleInputName" placeholder="Enter name">
					<input type="hidden" name="adminid" value="<?= $admin["AdminID"]; ?>"/>
				  </div>
				  <div class="form-group">
                    <label for="exampleInputSurname">Surname</label>
                    <input required type="text" class="form-control" name="user_surname" value="<?= $admin["Surname"]; ?>" id="exampleInputSurname" placeholder="Enter surname">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword">Password</label>
					<input type="password" class="form-control" name="user_password" id="exampleInputPassword" placeholder="Password">
					<input type="hidden" name="user_password_old" value="<?= $admin["Password"]; ?>" placeholder="Password">
				  </div>
				  <div class="form-group">
                    <label for="exampleInputEmail1">Email address</label>
                    <input required type="email" class="form-control" name="user_email" value="<?= $admin["Email"]; ?>" id="exampleInputEmail1" placeholder="Enter email">
                  </div>
				  ...
				  <div class="input-group">
                    ...
                    <input required type="text" name="user_phone" value="<?= $admin["Phone"]; ?>" class="form-control" data-inputmask='"mask": "(999) 999-9999"' data-mask>
                  </div>
                </div>
                <!-- /.card-body -->

                <div class="card-footer">
                  <button type="submit" name="edit_admin_btn" class="btn btn-danger savebtn">Save Changes</button>
                </div>
            </div>
            <!-- /.card -->
                ...
                <div class="card-body">
                  <div class="form-group">
					...
					<select hidden id="user_status_prev" name="user_status_prev" class="form-control">
                      <option name="simple">Simple User</option>
                      <option name="admin" selected>Administrator</option>
                    </select>
...
<script>
function deleteadmin_function() {
	var display_text = "Delete the admin " + $('#exampleInputName').val() + " " + $('#exampleInputSurname').val() + "?";
if (confirm(display_text)) {
		$('#action').val("Delete");
        document.getElementById("deleteadmin_form").submit();
    }else{
		$('#action').val("");
	}
}
