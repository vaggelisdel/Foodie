<?php
	...
	if ($_SESSION['admin_authorised'] == 0){
		...
		$result_all_recipes = mysqli_query($connect, $query);
	}
?>
<!DOCTYPE html>
<html>
		...
		<li class="nav-item">
		  ...
		  <li class="nav-item has-treeview menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="all_recipes.php" class="nav-link active">
                  <i class="fa fa-circle-o nav-icon"></i>
                  <p>All Recipes</p>
                </a>
              </li>
            ...
            <div class="card card-danger">
              ...
              <form role="form">
               ...
               <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php  
                    while($row = mysqli_fetch_array($result_all_recipes))  
						{
				?>
							<tr>
							  <td><?= $row["Title"] ?></td>
							  ...
							  <td><?php
								...
								$iduser = $row["UserID"];
								  if ($row["UserID"] == NULL){
									  $query ="SELECT Name, Surname FROM admins WHERE AdminID = '$idamdin'";  
									  $creator = mysqli_query($connect, $query);
									  $admincreator = mysqli_fetch_array($creator);
										echo $admincreator["Name"]." ".$admincreator["Surname"];
								  }else{
									  $query ="SELECT Name, Surname FROM users WHERE UserID = '$iduser'";  
									  $creator = mysqli_query($connect, $query);
									  $usercreator = mysqli_fetch_array($creator);
										echo $usercreator["Name"]." ".$usercreator["Surname"];
								  }?>
							   </td>
							  <td><?= $row["Description"] ?></td>
							  <td><center><a href="edit_recipe.php?&recipeid=<?= $row["RecipeID"] ?>"><button type="button" class="btn btn-block btn-outline-primary btn-sm">Edit</button></a></center></td>
							</tr>
				<?php
						}
				?>
