...
if (isset($_POST["create_recipe_btn"])){
    ...
    $sql = "INSERT INTO `recipes` (`UserID`, `Title`, `Time`, `Cost`, NoDishes, `Description`) 
	                     VALUES ('$userid', '$recipe_name', '$recipe_time', '$recipe_cost', '$recipe_dishes', '$recipe_description');";
    $connect->query($sql);

    //select this recipe for editing
    $result = $connect->query("SELECT * FROM recipes WHERE `UserID`='$userid' AND `Title`='$recipe_name'");
